...
## 🪄 Tell the AI what to change — the command box in the question creator (v1.35.0)
...
## ⏳ A batch with a RELEASE DATE on it (v1.39.0)
...
`RAPID_RELEASE_KEY` (`enRapidRelease`) / `rapidRelease` / `setRapidRelease` /
...
- After touching **the crop's pixel passes** (`_inkThreshold`, `INK_RATIO`,
  ...
  **`node tools/crop-tighten-tests.mjs`** and check a crop of a
  ...
  in the app sits in a band of blank paper again, which is the fault v1.42.0
...
- Run the twenty-five harnesses after touching what they cover — every failure they
